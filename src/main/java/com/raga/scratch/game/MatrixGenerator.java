...
public class MatrixGenerator {
	...
	public static List<List<String>> generateRandomMatrix() {
		...
		int totalStandardProbabilities = ScratchGameUtility.standardSymbolProbabilities.stream()
				.mapToInt(prob -> prob.symbolProbabilities.values().stream().mapToInt(Integer::intValue).sum()).sum();

		int totalBonusProbabilities = ScratchGameUtility.bonusSymbolProbabilities.values().stream().mapToInt(Integer::intValue).sum();

		List<String> symbolList = new ArrayList<>(ScratchGameUtility.symbols.keySet());
		for (int i = 0; i < 3; i++) {
